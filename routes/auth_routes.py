...
def index():
    ...
    if request.method == "POST":
        ...
        if action == "login":
            ...
            conn = get_connection()
            row = conn.execute(
                "SELECT suspended, suspend_reason FROM users WHERE username = ?", (username,)
            ).fetchone()
            conn.close()
            if row and row["suspended"]:
                reason = row["suspend_reason"] or "Contact support for details."
                return render_template(
                    "auth.html",
                    error=f"This account is suspended. {reason}"
                )
